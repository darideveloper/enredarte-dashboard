# subscription-emails Specification

## Purpose
Transactional emails that notify the artist and the admin list when an online (Stripe) subscription transitions into the `canceling` or terminal `canceled` state. Sends are transition-gated (at most once per transition and per `event_id`), best-effort, and ignore cash rows, reusing the shared notifications mailer and per-audience template convention.
## Requirements
