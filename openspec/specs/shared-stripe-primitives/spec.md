# shared-stripe-primitives Specification

## Purpose
Domain-neutral Stripe primitives shared by the subscription and artwork-sales domains: the `StripeEvent` idempotency/audit model, low-level SDK compatibility helpers, a single SDK initialization entry point, and the import-direction rule that keeps the two domains free of a module-level import cycle.
## Requirements
