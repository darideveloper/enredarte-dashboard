# Docs Formatting Specification

## Purpose
To define the requirements for consistent formatting across `docs/`: code fences must enclose code only with no orphaned prose, and wiki-link conventions must be uniform (short-form `[[name|label]]` for sibling docs, plain text labels for external vault resources).
