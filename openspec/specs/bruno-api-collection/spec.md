...
## Purpose
To define a Bruno API collection committed to the repository so developers can exercise the REST API (public catalog endpoint and authenticated DRF router root) against the local dev server without hard-coding hostnames or credentials. The workspace lives in `bruno/` as plain-text `.bru` files, uses a `dev.bru` environment exposing `base_url` and `token` variables, and ships a `bruno/README.md` explaining how to open the workspace and obtain a DRF Token.
