...
This specification defines 10 read-only, paginated, authenticated API endpoints under `/api/artworks/` — one per domain model in the `artworks` app — replacing the legacy monolithic `GET /api/catalog/`, plus public endpoints defined by the artwork-sales specification (buy / order summary / delivery) and the artwork-view-tracking specification (visit).
