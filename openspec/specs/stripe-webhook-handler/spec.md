...
The system SHALL dispatch each handled event inside a single Django `transaction.atomic()` block so that the `ArtistSubscription.upsert` and `Artist.is_active` update commit together. Any exception MUST roll the transaction back; the `StripeEvent` row outside the transaction MUST still record the error so debugging survives the 500 response.
...
- **THEN** the database SHALL roll back the partial write, `StripeEvent.error` SHALL contain the exception, the endpoint SHALL return HTTP 500, and Stripe SHALL retry the delivery.
...
The system SHALL dispatch events to one handler per Stripe event type. The minimum handled types are: `checkout.session.completed`, `customer.subscription.created`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_succeeded`, `invoice.payment_failed`.
...
- **THEN** the handler SHALL set `status="active"` and refresh `current_period_end`.

### Requirement: Correlation by stripe_customer_id and metadata
The system SHALL correlate Stripe events to local `ArtistSubscription` rows via `stripe_customer_id` first, and via `metadata.artist_id` (set on Checkout Session creation) for `checkout.session.completed` events that arrive before the customer id is known locally.
