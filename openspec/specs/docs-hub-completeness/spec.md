# Docs Hub Completeness Specification

## Purpose
To define the requirements for `docs/django.md` acting as a complete hub: it indexes every markdown document in `docs/` so no document is orphaned, and it documents the wikilink portability convention agents must follow when copying the docs into new Django projects.
