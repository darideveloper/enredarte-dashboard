# Docs Deploy Admin Coherence Specification

## Purpose
To define the requirements for consistency between the deployment and admin-wiring documentation: the container entrypoint runs `base_loaddata` and validates migrations, `project.admin` is imported explicitly in `urls.py`, and `project/admin.py` has a single canonical copy across `docs/`.
