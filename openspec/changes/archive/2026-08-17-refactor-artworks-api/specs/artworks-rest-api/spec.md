...
The system SHALL expose `GET /apis/artworks/artists/` (list) and `GET /apis/artworks/artists/{id}/` (detail). The queryset SHALL filter `is_active=True`. The list response SHALL be paginated. Each artist entry SHALL include all person fields (`id`, `slug`, `is_active`, `sort_order`, `created_at`, `updated_at`, `name`, `email`, `website`, `photo`, `birth_year`, `death_year`), a `location` reference as `{id, slug}` (or `null`), translations as `{es: {bio}, en: {bio}}`, and `social_links` as an array of `{id, platform, url}`.

#### Scenario: Artist list is paginated
- **WHEN** `GET /apis/artworks/artists/` is requested
- **THEN** the response SHALL be a paginated envelope with `count`, `next`, `previous`, `page`, `page_size`, `total_pages`, and `results` containing artist objects ordered by `sort_order`.
...
The system SHALL expose `GET /apis/artworks/art-curators/` (list) and `GET /apis/artworks/art-curators/{id}/` (detail). The queryset SHALL filter `is_active=True`. Each entry SHALL include `id`, `slug`, `is_active`, `sort_order`, `created_at`, `updated_at`, `name`, `email`, `website`, `photo`, and translations as `{es: {bio}, en: {bio}}`.
...
The system SHALL expose `GET /apis/artworks/locations/` (list) and `GET /apis/artworks/locations/{id}/` (detail). The queryset SHALL filter `is_active=True`. Each entry SHALL include `id`, `slug`, `is_active`, `sort_order`, `created_at`, `updated_at`, and translations as `{es: {name}, en: {name}}`.
...
The system SHALL expose `GET /apis/artworks/galleries/` (list) and `GET /apis/artworks/galleries/{id}/` (detail). The queryset SHALL filter `is_active=True`. Each entry SHALL include `id`, `slug`, `is_active`, `sort_order`, `created_at`, `updated_at`, `logo`, a `curator` reference as `{id, slug}` (or `null`), translations as `{es: {name, description}, en: {name, description}}`, and `artwork_links` as an array of `{id, artwork: {id, slug}, sort_order}`.
...
- **THEN** the response SHALL contain `id`, `slug`, `is_active`, `sort_order`, `created_at`, `updated_at`, and `translations` as `{es: {name}, en: {name}}`.

### Requirement: Artwork endpoint
The system SHALL expose `GET /apis/artworks/artworks/` (list) and `GET /apis/artworks/artworks/{id}/` (detail). The queryset SHALL filter `is_active=True`. Each entry SHALL include all BaseModel fields plus `artist` as `{id, slug}`, `year`, `dimensions`, `disciplines`, `techniques`, `themes`, `formats`, `scales` each as an array of `{id, slug}` objects, `price_mxn` and `price_usd` as numbers, `status` as its raw choice value, `is_highlighted`, `views_count`, translations as `{es: {title, description}, en: {title, description}}`, `images` as an array of `{id, image, alt_es, alt_en, is_primary, sort_order}`, and `gallery_links` as an array of `{id, gallery: {id, slug}, sort_order}`.

#### Scenario: Artwork list paginated
- **WHEN** `GET /apis/artworks/artworks/` is requested
- **THEN** the response SHALL be a paginated list of artwork objects with all fields described above, ordered by `sort_order`.
...
Every viewset queryset SHALL include `.filter(is_active=True)`. Inactive rows SHALL never appear in any API response.
...
All image fields (`Artist.photo`, `ArtCurator.photo`, `Gallery.logo`, `ArtworkImage.image`) SHALL be serialized as absolute URLs using `get_media_url()` from `utils/media.py`. The project SHALL define `HOST` in `project/settings.py` (from the `HOST` environment variable) so the local-prefix branch of `get_media_url()` works.
