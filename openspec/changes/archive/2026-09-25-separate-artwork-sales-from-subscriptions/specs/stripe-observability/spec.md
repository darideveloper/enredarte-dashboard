...
- **THEN** the system SHALL log at `INFO` the `event_id` and `event_type`, and the `core.models.StripeEvent` row SHALL be persisted with `processed_at=now()`.
...
- **THEN** `core/stripe.py` SHALL perform the `STRIPE_*` presence check on import and `subscriptions/apps.py:ready` SHALL retain its fail-fast `ImproperlyConfigured` gate, preventing silent `400` on all webhooks (`stripe.Webhook.construct_event(..., "")`) and silent `stripe.api_key=None` drift on any path (including artwork-only management commands that import `core.stripe` without importing subscription code).
